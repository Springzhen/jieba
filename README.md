修改说明
========
背景：在生产环境下用户自定义词典过大而导致单次调用时间太长
方案：基于flask 框架， 采用web lazy load 加载自定义词库到内存，实现快速调用的目的
问题：web service 加载中，如何实现词库动态更新问题
最终方案：将自定义词库分为两部分，一部分预先加载内存，另一部分为少量更新词库不预先加载，从而实现在快速调用的同时实现词库更新。
